...
if (!function_exists('apffw_draw_select_childs')) {

    function apffw_draw_select_childs(&$collector, $taxonomy_info, $term_id, $tax_slug, $childs, $level, $show_count, $show_count_dynamic, $hide_dynamic_empty_pos) {
        ...
        <?php if (!empty($childs)): ?>
            <?php foreach ($childs as $term) : ?>
                ...
                ?>
                <option <?php if ($show_count AND $count == 0 AND!in_array($term['slug'], $current_request)): ?>disabled=""<?php endif; ?> value="<?php _e($term['slug']);?>" <?php _e(selected(in_array($term['slug'], $current_request)));?> class="apffw-padding-<?php _e($level);?>"><?php _e(str_repeat('', $level));?><?php
                ...
                if (!empty($term['childs'])) {
                    apffw_draw_select_childs($collector, $taxonomy_info, $term['term_id'], $tax_slug, $term['childs'], $level + 1, $show_count, $show_count_dynamic, $hide_dynamic_empty_pos);
                }
                ?>
            <?php endforeach; ?>
        <?php endif; ?>
...
?>
<select class="apffw_select apffw_select_<?php _e($tax_slug);?>" name="<?php _e($this->check_slug($tax_slug));?>">
    <option value="0"><?php _e(APFFW_HELPER::wpml_translate($taxonomy_info));?></option>
    <?php
    ...
    $terms = apply_filters('apffw_sort_terms_before_out', $terms, 'select');
    ...
    <?php if (!empty($terms)): ?>
        <?php foreach ($terms as $term) : ?>
            ...
            if (!empty($term['childs'])) {
                apffw_draw_select_childs($collector, $taxonomy_info, $term['term_id'], $tax_slug, $term['childs'], 1, $show_count, $show_count_dynamic, $hide_dynamic_empty_pos);
            }
            $shown_options_tags++;
            ?>
        <?php endforeach; ?>
