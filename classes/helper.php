...
final class APFFW_HELPER {

    static $notices = array();

    public static function log($string) {
        $handle = fopen(APFFW_PATH . 'log.txt', 'a+');
        $string .= PHP_EOL;
    ...
    public static function get_terms($taxonomy, $hide_empty = true, $get_childs = true, $selected = 0, $category_parent = 0) {
        ...
        if (isset($APFFW->settings['cache_terms']) AND $APFFW->settings['cache_terms'] == 1) {
            $cache_key = 'apffw_terms_cache_' . md5($taxonomy . '-' . (int) $hide_empty . '-' . (int) $get_childs . '-' . (int) $selected . '-' . (int) $category_parent . '-' . $lang_key);
        ...
        $orderby = apply_filters('apffw_get_terms_orderby', $taxonomy);
        if (!empty($orderby)) {
            if ($orderby != $taxonomy) {
                $args['orderby'] = $orderby;
            }
        }
        $order = apply_filters('apffw_get_terms_order', $taxonomy, $orderby);
        if (!empty($order)) {
            if ($order != $taxonomy) {
                $args['order'] = $order;
            }
        ...
        $args = apply_filters('apffw_get_terms_args', $args);
        ...
        if (isset($APFFW->settings['cache_terms']) AND $APFFW->settings['cache_terms'] == 1) {
            ...
            if (isset($APFFW->settings['cache_terms_auto_clean'])) {
                $period = $APFFW->settings['cache_terms_auto_clean'];
    ...
    public static function wpml_translate($taxonomy_info, $string = '', $index = -1) {
        ...
        if (empty($string)) {
            if (is_object($taxonomy_info)) {
                $string = stripcslashes($APFFW->settings['custom_tax_label'][$taxonomy_info->name]);
            }
        }
        ...
        if (class_exists('SitePress') OR class_exists("Polylang")) {
            ...
            $apffw_settings = get_option('apffw_settings');
            if (isset($apffw_settings['wpml_tax_labels']) AND!empty($apffw_settings['wpml_tax_labels'])) {
                $translations = $apffw_settings['wpml_tax_labels'];
    ...
    public static function price_filter($additional_taxes = "") {
        global $_chosen_attributes, $wpdb, $wp, $APFFW;
        $request = $APFFW->get_request_data();

        $min_price = $APFFW->is_isset_in_request_data('min_price') ? esc_attr($request['min_price']) : '';
        $max_price = $APFFW->is_isset_in_request_data('max_price') ? esc_attr($request['max_price']) : '';
        ...
        $price_slider_html = '<form method="get" action="' . esc_url($form_action) . '">
			<div class="price_slider_wrapper">
				...
				<div class="price_slider_amount">
					<input type="text" id="min_price" name="min_price" value="' . esc_attr(apply_filters('woocommerce_price_filter_widget_amount', $min_price)) . '" data-min="' . esc_attr(apply_filters('woocommerce_price_filter_widget_amount', $min)) . '" placeholder="' . esc_html__('Min price', 'apffw-products-filter') . '" />
					<input type="text" id="max_price" name="max_price" value="' . esc_attr(apply_filters('woocommerce_price_filter_widget_amount', $max_price)) . '" data-max="' . esc_attr(apply_filters('woocommerce_price_filter_widget_amount', $max)) . '" placeholder="' . esc_html__('Max price', 'apffw-products-filter') . '" />
					<button type="submit" class="button">' . esc_html__('Filter', 'apffw-products-filter') . '</button>
					<div class="price_label" style="display:none;">
						' . esc_html__('Price:', 'apffw-products-filter') . ' <span class="from"></span> &mdash; <span class="to"></span>
					</div>
					' . $fields . '
					<div class="clear"></div>
				</div>
        ...
        $price_slider_html = apply_filters('apffw_price_slider_html', $price_slider_html, $price_slider_data);

        _e($price_slider_html);
    }

    public static function get_price2_filter_data($additional_taxes = '') {
        $apffw_settings = get_option('apffw_settings', array());
        if (isset($apffw_settings['by_price']['ranges']) AND!empty($apffw_settings['by_price']['ranges'])) {
            global $APFFW;
            $res = array();
            $request = $APFFW->get_request_data();
            $res['selected'] = '';
            ...
            $r = array();
            $rc = array();
            $ranges = explode(',', trim($apffw_settings['by_price']['ranges']));
            $get = $request;
            ...
            foreach ($ranges as $value) {
                ...
                $r[$key] = $value;
                if ($show_count) {
                    $rc[$key] = $APFFW->dynamic_count(NULL, 'none', $additional_taxes);
                } else {
                    $rc[$key] = 0;
                }
            }
    ...
    public static function set_layered_nav_product_ids() {
        if (isset($_REQUEST['apffw_wp_query_ids'])) {
            if ((defined('DOING_AJAX') && DOING_AJAX) AND isset($_REQUEST['predict_ids_and_continue'])) {        
                WC()->query->layered_nav_product_ids = sanitize_text_field($_REQUEST['apffw_wp_query_ids']);
            }
        }
    }

    public static function get_filtered_price($additional_taxes = "") {
        global $wpdb, $wp_the_query, $APFFW;
		...
		$current_term = $APFFW->get_really_current_term();
        ...
        $sql = apply_filters('apffw_get_filtered_price_query', $sql);

        if (isset($APFFW->settings['price_transient']) AND $APFFW->settings['price_transient']) {
            $data_key = md5($sql . 'apffw');
            $data = get_transient('apffw_min_max_prices');
            ...
            set_transient('apffw_min_max_prices', $data, 1 * 24 * 3600);
        }
    ...
    public static function get_max_price($additional_taxes = "") {
        ...
        if (version_compare(WOOCOMMERCE_VERSION, '2.6', '>')) {
            ...
            if (0 === sizeof(WC()->query->layered_nav_product_ids)) {
                ...
                $query_txt = self::apffw_prepare('SELECT max(meta_value + 0) FROM %1$s LEFT JOIN %2$s ON %1$s.ID = %2$s.post_id
    ...
    public static function show_admin_notice($key) {
        global $APFFW;
        _e($APFFW->render_html(APFFW_PATH . 'views/notices/' . $key . '.php'));
    }

    public static function add_notice($key) {
        $notices = get_option('apffw_notices', array());
        $is_hidden = false;
        ...
        if (!$is_hidden) {
            self::$notices[] = $key;
            $notices[] = $key;
            update_option('apffw_notices', $notices);
        }
    }

    public static function hide_admin_notices() {
        if (isset($_GET['apffw_hide_notice']) && isset($_GET['_wpnonce'])) {
            if (!wp_verify_nonce($_GET['_wpnonce'])) {
                wp_die(__('Action failed. Please refresh the page and retry.', 'apffw-products-filter'));
            }

            if (!current_user_can('manage_woocommerce')) {
                wp_die(__('Cheatin&#8217; huh?', 'apffw-products-filter'));
            }

            $key = self::escape(sanitize_text_field($_GET['apffw_hide_notice']));
    ...
    public static function draw_tooltipe($title, $tooltip_text) {
        ...
        if (!isset($APFFW->settings['use_tooltip'])) {
            $show_tooltip = 1;
        } else {
            $show_tooltip = $APFFW->settings['use_tooltip'];
        }
        if (!$show_tooltip) {
            return"";
        }
        ...
        $toggle_image = ((isset($APFFW->settings['apffw_tooltip_img']) AND!empty($APFFW->settings['apffw_tooltip_img'])) ? $APFFW->settings['apffw_tooltip_img'] : APFFW_LINK . 'img/apffw_info_icon.png');
        $current_id = uniqid("apffw_tooltip_content");
        ?>
        <img src="<?php _e($toggle_image);?>" class="apffw_tooltip_header" data-tooltip-content="#<?php _e($current_id);?>">
        <div class="apffw_tooltip_templates">
            <span id="<?php _e($current_id);?>">
                <span class="apffw_tooltip_title"><?php _e($title);?></span>
                <span class="apffw_tooltip_text"><?php _e($tooltip_text);?></span>
            </span>
        </div>          
        <?php
    }
    ...
    public static function draw_title_toggle($show, $block_is_closed) {
        ...
        global $APFFW;
        $condition = 'closed';
        $toggle_type = ((isset($APFFW->settings['toggle_type']) AND!empty($APFFW->settings['toggle_type'])) ? $APFFW->settings['toggle_type'] : 'text');

        if ($block_is_closed) {
            $toggle_text = ((isset($APFFW->settings['toggle_closed_text']) AND!empty($APFFW->settings['toggle_closed_text'])) ? self::wpml_translate(null, $APFFW->settings['toggle_closed_text']) : '-');
            $toggle_image = ((isset($APFFW->settings['toggle_closed_image']) AND!empty($APFFW->settings['toggle_closed_image'])) ? $APFFW->settings['toggle_closed_image'] : APFFW_LINK . 'img/plus3.png');
        } else {
            $toggle_text = ((isset($APFFW->settings['toggle_opened_text']) AND!empty($APFFW->settings['toggle_opened_text'])) ? self::wpml_translate(null, $APFFW->settings['toggle_opened_text']) : '+');
            $toggle_image = ((isset($APFFW->settings['toggle_opened_image']) AND!empty($APFFW->settings['toggle_opened_image'])) ? $APFFW->settings['toggle_opened_image'] : APFFW_LINK . 'img/minus3.png');
            $condition = 'opened';
        }
        ...
        if ($toggle_type == 'text' OR empty($toggle_image)) {
            ?>
            <a href="javascript: void(0);" title="<?php esc_html_e('toggle', 'apffw-products-filter') ?>" class="apffw_front_toggle apffw_front_toggle_<?php _e($condition);?>" data-condition="<?php _e($condition);?>"><?php _e($toggle_text);?></a>
            <?php
        } else {
            ?>
            <a href="javascript: void(0);" title="<?php esc_html_e('toggle', 'apffw-products-filter') ?>" class="apffw_front_toggle apffw_front_toggle_<?php _e($condition);?>" data-condition="<?php _e($condition);?>">
                <img src="<?php _e($toggle_image);?>" alt="<?php esc_html_e('toggle', 'apffw-products-filter') ?>" />
            </a>
            <?php
        }
    }
    ...
    public static function draw_more_less_button($type) {
        $args = apply_filters('apffw_get_more_less_button_' . $type, array());
        if (empty($args)) {
            $args['type'] = 'text';
            $args['closed'] = esc_html__('Show more', 'apffw-products-filter');
            $args['opened'] = esc_html__('Show less', 'apffw-products-filter');
        }

        if ($args['type'] == 'image') {
            ?>
            <a href="javascript:void(0);" class="apffw_open_hidden_li_btn" data-type="<?php _e($args['type']);?>" data-state="closed" data-closed="<?php _e($args['closed']);?>" data-opened="<?php _e($args['opened']);?>"><img src="<?php _e($args['closed']);?>" alt="" /></a>
            <?php
        } else {
            ?>
            <a href="javascript:void(0);" class="apffw_open_hidden_li_btn" data-type="<?php _e($args['type']);?>" data-state="closed" data-closed="<?php _e($args['closed']);?>" data-opened="<?php _e($args['opened']);?>"><?php _e($args['closed']);?></a>
            <?php
        }
    }
    ...
    public static function apffw_prepare($query, $args) {
        global $wpdb;
        return $wpdb->prepare($query, $sql_val);
        ...
        foreach ($args as $item) {
            ...
            $sql_val[] = self::apffw_escape_sql($item['type'], $item['val']);
        }
        return @aprintf($query, $sql_val);
    }

    public static function apffw_escape_sql($type, $value) {
        switch ($type) {
            case'string':
                global $wpdb;
